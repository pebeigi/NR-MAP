...
from traffic_env import EnvConfig
...
def calibrate(
    ...
    scored: list[tuple[float, float, float, float, dict[str, float]]] = []
    log_progress(
        f"Global calibration: evaluating {len(candidates)} candidates with "
    ...
    for i, (_, params) in enumerate(candidates):
        ...
        scored.append((objective, one_step_loss, closed_loop_loss, tracking_loss, params))
        if args.verbose and (
            (i + 1) % max(len(candidates) // 10, 1) == 0 or i + 1 == len(candidates)
        ):
            best = min(scored, key=lambda x: x[0])
            log_progress(
                f"  closed-loop: {i + 1}/{len(candidates)} candidates, "
    ...
    top = scored[:top_n]
    top_matrix = {
        key: np.array([params[key] for _, _, _, _, params in top], dtype=float)
    ...
    ranges = {
        ...
        for key, values in top_matrix.items()
    }
    best_objective, best_nll, best_closed_loop_loss, best_tracking_loss, best_params = scored[0]
    return {
        "best_objective": best_objective,
        "best_nll": best_nll,
        "best_closed_loop_loss": best_closed_loop_loss,
        "best_tracking_rank": best_tracking_loss * max(args.tracking_rank_normalizer, 1.0),
        "best_params": best_params,
        "recommended_ranges_from_top_trials": ranges,
        "n_samples": len(samples),
        "n_rollout_windows": len(windows),
        "closed_loop_candidates": len(candidates),
...
def write_diagnostics(
    ...
    n_id_plots = plot_id_timeseries(
        ...
        global_params=result["best_params"],
        args=args,
    )
    log_progress("Diagnostics: writing calibration quality plots...", args.verbose)
    qdf = calibration_quality_frame(samples, result["best_params"], args.temperature)
...
def main() -> None:
    ...
    parser.add_argument("--csv", type=Path, default=Path("data/Final_Lebanon_Data.csv"))
    parser.add_argument("--output", type=Path, default=Path("calibration/utility_calibration.json"))
    ...
    parser.add_argument("--diagnostics-dir", type=Path, default=Path("calibration/diagnostics"))
    parser.add_argument(
        "--boundary-csv",
        type=Path,
        default=Path("data/derived_highway_boundaries/highway_boundaries.csv"),
